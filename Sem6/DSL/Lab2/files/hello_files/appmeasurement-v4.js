...
/* Specify the life time of the cookie in seconds, or set to "Session" to turn off persistent cookies. */
/* Quick overview: https://experienceleague.adobe.com/docs/analytics/implementation/js/overview.html */
s.cookieLifetime="Session"
/* Link Tracking Config */
s.trackDownloadLinks=true
s.trackExternalLinks=true
s.trackInlineStats=true
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,xlsm,ppt,pptx,mobi,epub,mp4,csv,jpg,png"
s.linkInternalFilters="javascript:,fdic.gov,mailto:"
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="None"

function fdicAnalyticsConversion(query, eVar) {
    if (!query || !query.trim() || !eVar || !eVar.trim()) return undefined;
    if (s && s.Util && s.Util.getQueryParam) {
        s[eVar] = s.Util.getQueryParam(query) || undefined;
    }
    return s[eVar];
}

function fdicAnalyticsEvent(event) {
    if (!event || !event.trim()) return undefined;
    s.events = s.events || "";
    if (s.events.indexOf(event) < 0) {
        s.events += (s.events.trim().length ? "," : "") + event;
    }
    return s.eventName;
}

if (fdicAnalyticsConversion("utm_campaign", "eVar5")) {
    fdicAnalyticsEvent("event5");
}
fdicAnalyticsConversion("utm_medium", "eVar7");
fdicAnalyticsConversion("utm_source", "eVar8");
...
function s_doPlugins(s) {
    ...
    const prPrefix = "FDIC: Speeches & Testimony - ";
    var isPR = title.startsWith(prPrefix);

    //if it is a PR, then trim off the prefix "FDIC: Speeches & Testimony - ""
    if (isPR){
      let prTitle= title.replace(prPrefix, "");
      s.pageName = prTitle; 
    } else {
...
0;a.offlineFilename="AppMeasurement.offline";a.Oa=0;a.ma=0;a.N=0;a.Na=0;a.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,jpg,png";a.w=h;a.d=h.document;try{if(a.Sa=!1,navigator){var v=navigator.userAgent;if("Microsoft Internet Explorer"==navigator.appName||0<=v.indexOf("MSIE ")||0<=v.indexOf("Trident/")&&0<=v.indexOf("Windows NT 6"))a.Sa=!0}}catch(w){}a.ga=function(){a.ha&&(h.clearTimeout(a.ha),a.ha=n);a.l&&a.J&&a.l.dispatchEvent(a.J);a.A&&("function"==typeof a.A?a.A():
