...
int kw(FILE *rd, int *r, int*c, struct token *arr, int *idx)
{
	int c2=0;
	char key[100];
	char words[16][10] ={"if","else","int","float","char","break","case","continue","default","double","while","return","sizeof","struct","void"};
    int i=0;
	char ch = getc(rd);
	c2++;
...
void id(FILE *rd, int*r, int*c, struct token *arr, int *idx,int *idc, int *sidx, struct symboltable *symt)
{
	...
	if(strcmp(symt[sum].lexemename, key)!=0)
	{
		...
		{
			int x = (*idx)-1;
			while(strcmp(arr[x].type,"int")!=0 && strcmp(arr[x].type,"float")!=0 && strcmp(arr[x].type,"double")!=0)
			{
				x--;
			}
